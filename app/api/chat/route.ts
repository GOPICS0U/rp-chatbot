...
export async function POST(request: Request) {
  try {
    ...
    let conversation=conversationId?await prisma.conversation.findUnique({where:{id:conversationId}}):null; if(!conversation || conversation.characterId!==characterId) conversation=await prisma.conversation.create({data:{characterId}});
    if(!regenerateId) await prisma.message.create({data:{role:'user',content,characterId,conversationId:conversation.id}});
    const history=await prisma.message.findMany({where:{conversationId:conversation.id},orderBy:{createdAt:'asc'},take:80});
